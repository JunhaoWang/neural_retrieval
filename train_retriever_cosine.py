...
class RetriverTrainer(pl.LightningModule):
    ...
    def step_helper_cosine(self, batch, eval=False):
        input_ids_question, attention_mask_question, token_type_ids_question, \
        batch_input_ids_paragraphs, batch_attention_mask_paragraphs, \
        batch_token_type_ids_paragraphs = batch
        ...
        if eval:
            all_dots = torch.bmm(h_question.repeat(num_document, 1).unsqueeze(1),
                                 h_paragraphs_batch.reshape(-1, self.emb_dim).unsqueeze(2)).reshape(batch_size,
                                                                                                    num_document)
        ...
        loss = torch.nn.CosineEmbeddingLoss()(
            h_question.repeat(num_document, 1), h_paragraphs_batch.reshape(-1, self.emb_dim), labs
        )
        return loss, all_prob
    ...
    def training_step(self, batch, batch_idx):
        ...
        train_loss, _ = self.step_helper_cosine(batch, False)
        # logs
    ...
    def validation_step(self, batch, batch_idx):
        loss, all_prob = self.step_helper_cosine(batch, True)
...
if __name__ == '__main__':
    ...
    checkpoint_callback = ModelCheckpoint(
        filepath='out/cosine-{epoch}-{val_loss:.2f}-{val_acc:.2f}',
        save_top_k=1,
        verbose=True,
        monitor='val_acc',
        mode='max'
